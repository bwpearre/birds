...
data = zeros(1, 4 + ntimes_of_interest);
data_lines = 0;
data_capacity = 1;

for i = 5%1:length(datadirs)
    ...
    for j = 1:length(datafiles)
        ...
        if use_swift
            s = system(sprintf('%s --net network_foo.mat --audio %s > out.csv', ...
                detectsyllables, ...
                filename));
            dcheck = dir('out.csv');
            if dcheck.bytes == 0
...
foo = bsxfun(@minus, datas(:, 5:end), net.trigger_thresholds);
...
transition_probabilities(datas, ntimes_of_interest);
