...
for channel = 1:nchannels
    ...
    for posneg = 1:length(peaks)
        ...
        if length(peaks{posneg}) >= min_peaks
            ...
            gmm_final = g{kclusts(posneg)};
            % How many spikes were observed within 200us of a cluster centre?
            for cluster = 1:kclusts(posneg)
                probs(cluster, channel, posneg) = sum(abs(peaks{posneg} - gmm_final.mu(cluster)) < 0.0002);
            end
        end
    end
end
